...
from discord import AllowedMentions, CustomActivity, AutoShardedClient as AB, Intents, Interaction, Message, Embed
from .context import PretendContext, PretendHelp, PretendInteraction
from asyncpg import create_pool
from discord.ext import commands
from .ext import Client, HTTP
...
class Pretend(commands.AutoShardedBot):
    def __init__(self):
        super().__init__(
            ...
            intents=intents,
            shard_count=3,
            allowed_mentions=AllowedMentions(roles=False, everyone=False, users=True, replied_user=False),
        ...
        self.main_guilds = [1258712533284683798, 1246416801311625308, 1266088675880472669]
    ...
    # Handle the mention and display the prefix
        if message.content == "<@{}>".format(self.user.id):
            prefix_text = "Prefix is " if len(await self.prefixes(message)) == 1 else "Your prefixes"
            return await message.reply(embed=discord.Embed(color=self.color, description=f"{prefix_text}: {prefixes}"))
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
      if isinstance(error, commands.CommandNotFound): return 
        ...
        if isinstance(error, commands.MissingPermissions): return await ctx.send_warning(f"You do not have the **required** permission: `{error.missing_permissions[0]}` to **proceed** with this **action**.")
      elif isinstance(error, commands.CommandOnCooldown):
        if ctx.command.name != "hit": return await ctx.reply(embed=discord.Embed(color=0xefbc1b, description=f"{ctx.author.mention}: You must wait  **{format_timespan(error.retry_after)}** before attempting to use **{ctx.command.qualified_name}** again to prevent further conflicts."), mention_author=False)    
       ...
       await self.ext.send_warning(ctx, f"A **system** error has occurred. To **expedite resolution**, please join our [**support server**](https://discord.gg/blonde) and share the **following reference** code: `{key}`.  Our **team** will assist you **promptly**.")   
...
async def disabled_command(ctx: commands.Context):
  ...
  if check: await bot.ext.send_warning(ctx, f"The **execution** of the command **{cmd.name}** has been **restricted** in this **guild** at the discretion of the **administrator**.")     
...
bot.run("MTI2MzczNDk1ODU4NjA3MzE0MQ.GwKpfw.K8oiH2T4civH5kULmDg5FF2L0spCEW55md1o_Y")
