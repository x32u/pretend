...
class Webhooks(Cog):
    ...
    @webhook_editor.command(description="Create a webhook in a channel", usage="#channel",name="create", brief="manage webhooks")
    @has_guild_permissions(manage_webhooks=True)
    @bot_has_guild_permissions(manage_webhooks=True)
    async def webhook_create(self, ctx, channel: discord.TextChannel, *, name: str = None):
      """
      config
      """
    ...
    @webhook_editor.group(help="config",
    ...
    @webhook_edit.command(description="Edit a webhook's name", usage="[code] [name]",name="name", brief="manage webhooks")
    @has_guild_permissions(manage_webhooks=True)
    async def webhook_edit_name(
        self, ctx, code: ValidWebhookCode, *, name: str
    ):
        """
        config
        """
    ...
    @webhook_edit.command(help="config", description="edit a webhook avatar", usage="[code] [avatar url]",name="avatar", aliases=["icon"], brief="manage webhooks")
    @has_guild_permissions(manage_webhooks=True)
    async def webhook_edit_avatar(
    self, ctx, code: ValidWebhookCode, url: str = None
):
      if not url:
    ...
    @webhook_editor.command(description="Send a webhook", usage="[code] [discohook json file / embed code / text]", name="send", brief="manage webhooks")
    @has_guild_permissions(manage_webhooks=True)
