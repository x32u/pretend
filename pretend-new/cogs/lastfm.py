import discord, json, typing, get.userhandler as userhandler, traceback, asyncio
from discord.ext import commands, tasks  
from get.utils import EmbedBuilder
from get.lastfmhandler import Handler
...
async def lastfm_message(ctx: commands.Context, content: str) -> discord.Message: 
 return await ctx.reply(embed=discord.Embed(color=0xff0000, description=f"> <:link:1243406673184424016> {ctx.author.mention}: {content}"))  
...
class Lastfm(commands.Cog):
    ...
    @commands.group(help="utility", invoke_without_command=True, aliases = ['lf'])
    ...
    async def lf_set(self, ctx: commands.Context, *, ref: str):
      ...
      check2 = await self.bot.db.fetchrow('SELECT username FROM lastfm WHERE user_id = $1', ctx.author.id)
      ...
      mes = await ctx.send("Get user history... :gear:")
      await asyncio.sleep(1) 
      embed = discord.Embed(color=self.bot.color, description=f"> Connected to **{ref}** 🎸")           
    ...
    async def lf_embed_set(self, ctx: commands.Context, *, embed: str):   
     ...
     if any(invite in ctx.message.content for invite in invites): 
      await ctx.send_warning("You cannot have discord invite links in your **last.fm** embed")
        ...
        try:
            ...
            if check:
                ...
                if user != "error":
                    ...
                    embed.add_field(
                        name=f"**{a['recenttracks']['track'][0]['name'].upper()}**",
                        value=f"by [{a['recenttracks']['track'][0]['artist']['#text']}](https://last.fm/music/{artist})\non {a['recenttracks']['track'][0]['album']['#text']}",
                        inline=False
                    )
                    ...
                    # Capture the message object when sending the embed
                ...
                try:
                    ...
                    message = await ctx.send(await self.lastfm_replacement(user, starData[0]))
                return await lf_add_reactions(ctx, message)
            elif check is None:
                return await lastfm_message(ctx, f"**{member}** doesn't have a **Last.fm account** linked. Use `{ctx.clean_prefix}lf set <username>` to link your **account**.")
        except Exception:
