...
import discord, datetime, io, colorgram, uwuipy, timezonefinder, humanfriendly, humanize, arrow
...
class Timezone(object):
  ...
  async def timezone_send(self, ctx: Context, message: str):
   return await ctx.reply(embed=discord.Embed(color=self.bot.color, description=f"> {ctx.author.mention}: {message}"))
  ...
  async def get_user_timezone(self, ctx: Context, member: discord.Member): 
   if await self.timezone_request(member) is None: 
    if member.id == ctx.author.id: return await ctx.send_warning( f"Use `{ctx.clean_prefix}tz set` first.")   
    else: return await ctx.send_warning( f"**{member.name}** doesn't have their **timezone** set")
   if member.id == ctx.author.id: return await self.timezone_send(ctx, f"Current time: **{await self.timezone_request(member)}**")
   else: return await self.timezone_send(ctx, f"Current time for **{member.name}** is **{await self.timezone_request(member)}**") 
...
class utility(commands.Cog):
    ...
    async def firstmessage(self, ctx: Context, *, channel: TextChannel=None):
     ...
     message = messages[0]
     embed = Embed(color=self.bot.color, title="first message in #{}".format(channel.name), description=f"> {message.content}", timestamp=message.created_at)
    ...
    async def bday_set(self, ctx: Context, month: str, day: str): 
     ...
     except: return await ctx.send_error(f"`{ctx.clean_prefix}birthday set [month] [day]`") 
...
{user.discriminator} - returns the user's discriminator
 ...
 {guild.boost_tier} - returns the server's boost level
   """
    ...
    @command(description="get a user info", help="utility", aliases=["user", "ui", "whois"])
    async def userinfo(
        self,
        ctx,
        *,
    ...
    @command(description="gets the banner from a server based by invite code", help="utility", usage="[invite code]")
    ...
    @command(description="gets the splash from a server based by invite code", help="utility", usage="[invite code]")
    ...
    @command(description="gets the icon from a server based by invite code", help="utility", usage="[invite code]")
    ...
    @command(help="utility", description="get a server info(you can put a invite code after)", aliases=["si", "server"])
    ...
    @command(help="utility", description="get a channel information", name="channelinfo", aliases=["ci"])
    async def channelinfo(
        self, ctx, *, channel: Optional[TextChannel] = None
    ):
        channel = channel or ctx.channel
    ...
    async def pin(self, ctx):
      if ctx.message.reference:
        ...
        return await ctx.send_error(f"Please reply to a **message** and type `{ctx.clean_prefix}pin`")
    ...
    async def picperms(self, ctx: Context, member: Member, *, channel: TextChannel=None):
      ...
      if channel.permissions_for(member).attach_files and channel.permissions_for(member).embed_links:
       ...
       return await ctx.send_success(f"{member.mention} can no longer send images in {channel.mention}")       
      await channel.set_permissions(member, attach_files=True,embed_links=True)
      return await ctx.send_success(f"{member.mention} can send now images in {channel.mention}")
    ...
    async def roleinfo(self, ctx: Context, *, role: Union[Role, str]): 
      if isinstance(role, str): 
        ...
        if not role: return await ctx.send_warning(f"**{role.name}** is not a valid role")
    ...
    async def snipe(self, ctx: Context, *, number: int=1):
        ...
        if len(check) == 0: return await ctx.send_error("🔎 No deleted messages found in this channel") 
    ...
    async def bans(self, ctx: Context): 
     ...
     if len(banned) == 0: return await ctx.send_warning("There are no banned people here")
     i=0
     k=1
     l=0
     mes = ""
