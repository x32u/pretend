...
class Messages(commands.Cog): 
      ...
      if (args[0] == "pretend"):
        ...
        if "tiktok" in url:
         async with message.channel.typing():  
          ...
          x = x["data"]
          embed.set_footer(text=f"❤ {self.bot.ext.human_format(x['digg_count'])}  💬 {self.bot.ext.human_format(x['comment_count'])}  🔗 {self.bot.ext.human_format(x['share_count'])}  👀 {self.bot.ext.human_format(x['play_count'])} | {message.author}", icon_url="https://media.discordapp.net/attachments/1145741060505612330/1156610676937211924/astars.png?ex=65159922&is=651447a2&hm=c7084c2a73ee93bf2db7a482c71fd92e092c0a585440962bbb76362f0add2b96&")
    ...
    async def afk_listener(self, message: discord.Message):
     ...
     if che:
      embed = discord.Embed(color=self.bot.color, description=f"> <:bd_butterfly:1258061124444749828> Welcome back **{message.author}**! You were AFK since **{self.bot.ext.relative_time(datetime.datetime.fromtimestamp(int(che['time'])))}**")
