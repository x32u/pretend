...
from get.checks import Perms, Mod
from get.utils import PaginatorView, GoodRole, NoStaff
...
class Premium(commands.Cog):
   ...
   async def perks(self, ctx: commands.Context): 
      embed = discord.Embed(color=self.bot.color, title="", description="> **embed steal** - steal member's lastfm custom embed\n" + '\n'.join([f"> **{command.name}** - {command.description}" for command in set(ctx.cog.walk_commands()) if not command.name in [ctx.command.name, 'donate'] and not "discrim" in command.name]))
