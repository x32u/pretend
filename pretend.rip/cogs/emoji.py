...
class Emoji(Cog):
    ...
    async def emojis(self, ctx: PretendContext):
        ...
        if len(ctx.guild.emojis) == 0:
            return await ctx.send_warning("There are **no** emojis in this server.")
    ...
    @command(aliases=["stickerlist"])
    async def stickers(self, ctx: PretendContext):
        """
        Returns a list of stickers in the server
        """

        if len(ctx.guild.stickers) == 0:
            return await ctx.send_warning("There are **no** stickers in this server.")

        await ctx.paginate(
            [f"[{sticker.name}]({sticker.url}) (`{sticker.id}`)" for sticker in ctx.guild.stickers],
