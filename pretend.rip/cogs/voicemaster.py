...
class Voicemaster(Cog):
    def __init__(self, bot: Pretend):
        ...
        self.values = [
            ("<:1226148245567176765:1260756781802717184>", "`lock` the voice channel"),
            ("<:1226148238411694120:1260756791030054912>", "`unlock` the voice channel"),
            ("<:1226148228593094676:1260756774978453605>", "`hide` the voice channel"),
            ("<:1226148230488920075:1260756776647786517>", "`reveal` the voice channel"),
            ("<:1226148225971523627:1260756787146264647>", "`rename` the voice channel"),
            ("<:1226148224264568892:1260756780116480021>", "`decrease` the member limit"),
            ("<:1226148222578200666:1260756789125972060>", "`increase` the member limit"),
            ("<:1226148220997210203:1260756785225007144>", "`info` about the voice channel"),
            ("<:1226148235127816232:1260756778417913949>", "`kick` someone from the voice channel"),
            ("<:1226148232464437259:1260756783547289680>", "`claim` the voice channel"),
        ]
    ...
    async def create_temporary_channel(
        ...
        try:
            ...
            await channel.delete(reason="VoiceMaster: Cleanup Unused Voice Channel")
            return
        ...
        async with self.locks[f"VOICEMASTER:{channel.id}:{member.id}"]:
            await asyncio.sleep(0.9)
            if len(channel.members) >= 1:
                return

            await self.delete_temporary_channel(channel)
