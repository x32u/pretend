...
class Owner(Cog):
    ...
    async def add_donor_role(self, member: discord.Member):
    ...
    async def guilds(self, ctx: PretendContext):
        ...
        return await ctx.paginate(
            [f"{g.name} ``{g.id}`` - {g.member_count:,} members" for g in servers],
            "pretend's servers",
        )
    ...
    @is_owner()
    async def donor_add(self, ctx: PretendContext, *, member: discord.Member):
        ...
        role = ctx.guild.get_role(1269389122263580783)

        try: await member.add_roles(role, reason="member got donator perks")
        except: pass
        await self.bot.db.execute(
    ...
    async def globalban(
        ...
        *,
        ...
        if user.id in [1188163689208090684, 852784127447269396, 1246380245272231987]:
    ...
    async def blacklist_server(self, ctx: PretendContext, *, server_id: int):
        """blacklist a server"""
        if server_id in [1258712533284683798]:
    ...
    @command()
    @is_owner()
    async def reskinreset(self, ctx: PretendContext, user: discord.User):
        """Reset a users reskin"""

        check = await self.bot.db.execute("SELECT * FROM reskin WHERE user_id = $1", user.id)
        if check is None: return await ctx.send_warning("This user **does not** have reskin enabled.")

        await self.bot.db.execute("DELETE FROM reskin WHERE user_id = $1", user.id)
        await ctx.send_success(f"I have deleted the reskin values for **{user.name}**.")
