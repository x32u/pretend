...
class Moderation(Cog):
    ...
    @role.command(name="all", brief="manage_roles")
    @has_guild_permissions(manage_roles=True)
    @bot_has_guild_permissions(manage_roles=True)
    ...
    @role.command(name="humans", brief="manage_roles")
    @has_guild_permissions(manage_roles=True)
    @bot_has_guild_permissions(manage_roles=True)
    async def role_humans(self, ctx: PretendContext, *, role: NewRoleConverter):
        """
        ...
        async with self.role_lock[ctx.guild.id]:
            tasks = [
                m.add_roles(role, reason=f"Role all invoked by {ctx.author}")
                for m in ctx.guild.members
                if not role in m.roles and not m.bot
            ]
    ...
    @role.command(name="humans remove", brief="manage_roles")
    @has_guild_permissions(manage_roles=True)
    @bot_has_guild_permissions(manage_roles=True)
    async def role_humans_remove(self, ctx: PretendContext, *, role: NewRoleConverter):
        """
        ...
        async with self.role_lock[ctx.guild.id]:
            tasks = [
                m.remove_roles(role, reason=f"Role remove all invoked by {ctx.author}")
                for m in ctx.guild.members
                if not role in m.roles and not m.bot
            ]

            if len(tasks) == 0:
                return await ctx.send_warning("There is **no one** with this role.")

            mes = await ctx.pretend_send(
                f"Removing {role.mention} from **{len(tasks)}** members. This operation might take around **{format_timespan(0.3*len(tasks))}**"
            )
            ...
            return await mes.edit(
                embed=Embed(
                    color=self.bot.yes_color,
                    description=f"{self.bot.yes} {ctx.author.mention}: Removed {role.mention} from **{len(tasks)}** members",
                )
            )
    ...
    @role.command(name="delete", aliases=["del"], brief="manage_roles")
    @has_guild_permissions(manage_roles=True)
    @bot_has_guild_permissions(manage_roles=True)
