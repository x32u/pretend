...
def is_autonsfw():
    async def predicate(ctx: PretendContext):
        if not await ctx.bot.db.fetchrow(
            "SELECT * FROM nsfw_toggle WHERE guild_id = $1", ctx.guild.id):
          await ctx.send_warning(f"Autonsfw is **not** toggled, consider using `{ctx.clean_prefix}autonsfw toggle` instead.")
          return False
        return True
    return check(predicate)
...
def is_autonsfw():
    async def predicate(ctx: PretendContext):
        if await ctx.bot.db.fetchrow(
            "SELECT * FROM nsfw_toggle WHERE guild_id = {}".format(ctx.guild.id)
        ):
            return True
...
def has_perks():
    async def predicate(ctx: PretendContext):
        ...
        if not check:
            await ctx.send_warning(
                f"You need [**donator**](https://discord.gg/7jUMQ6YnX5) perks to use this command"
            )
        return check is not None
