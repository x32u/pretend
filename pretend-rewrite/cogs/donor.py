...
from get.utils import GoodRole, NoStaff, PaginatorView
...
class GoToModal(discord.ui.Modal, title="change the page"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        attachments = (
            [
                discord.File(
                    fp=await self.bot.getbyte(self.files[int(self.page.value) - 1][1]),
                    filename="instagram.mp4",
                )
            ]
            if self.files[int(self.page.value) - 1][0] == "video"
            else [
                discord.File(
                    fp=await self.bot.getbyte(self.files[int(self.page.value) - 1][1]),
        ...
        await interaction.response.edit_message(
            embed=self.embeds[int(self.page.value) - 1], attachments=attachments
        )
...
class InstagramPaginatorView(discord.ui.View):
    ...
    @discord.ui.button(
        emoji=f"<:right:1018156484170883154>", style=discord.ButtonStyle.blurple
    )
    async def left(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.i == 0:
            attachments = (
                [
                    discord.File(
                        fp=await self.bot.getbyte(self.files[-1][1]),
                        filename="instagram.mp4",
                    )
                ]
                if self.files[-1][0] == "video"
                else [
                    discord.File(
                        fp=await self.bot.getbyte(self.files[-1][1]),
                        filename="instagram.png",
            ...
            await interaction.response.edit_message(
                embed=self.embeds[-1], attachments=attachments
            )
        ...
        attachments = (
            [
                discord.File(
                    fp=await self.bot.getbyte(self.files[self.i][1]),
                    filename="instagram.mp4",
                )
            ]
            if self.files[self.i][0] == "video"
            else [
                discord.File(
                    fp=await self.bot.getbyte(self.files[self.i][1]),
                    filename="instagram.png",
        ...
        return await interaction.response.edit_message(
            embed=self.embeds[self.i], attachments=attachments
        )
    ...
    async def right(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.i == len(self.embeds) - 1:
            attachments = (
                [
                    discord.File(
                        fp=await self.bot.getbyte(self.files[0][1]),
                        filename="instagram.mp4",
                    )
                ]
                if self.files[0][0] == "video"
                else [
                    discord.File(
                        fp=await self.bot.getbyte(self.files[0][1]),
                        filename="instagram.png",
            ...
            await interaction.response.edit_message(
                embed=self.embeds[0], attachments=attachments
            )
            self.i = 0
            return
        self.i = self.i + 1
        ...
        return await interaction.response.edit_message(
            embed=self.embeds[self.i], attachments=att
        )
    ...
    @discord.ui.button(
        emoji="<:stop:1018156487232720907>", style=discord.ButtonStyle.danger
    )
...
class Premium(commands.Cog):
            ...
            try:
                ...
                if len(webhooks) == 0:
                    webhook = await message.channel.create_webhook(
                        name="delta uwulock", reason="for uwulock"
                    )
                else:
                    webhook = webhooks[0]
                await webhook.send(
   ...
   """
    ...
    @commands.command(
        description="hardban | hardunban an user from the server",
        help="donor",
        usage="[user]",
        brief="ban_members",
    )
    ...
    async def hardban(
        ...
        if isinstance(member, discord.Member):
            ...
            if member.id == self.bot.user.id:
                return await ctx.reply("get back on your work and leave me alone..")
    ...
    async def perks(self, ctx: commands.Context):
        embed = discord.Embed(
            ...
            title="",
            description="> **embed steal** - steal member's lastfm custom embed\n"
            + "\n".join(
                [
                    f"> **{command.name}** - {command.description}"
                    for command in set(ctx.cog.walk_commands())
                    if not command.name in [ctx.command.name, "donate"]
