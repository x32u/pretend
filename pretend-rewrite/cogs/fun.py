...
class fun(commands.Cog):
    ...
    @commands.command(
        aliases=["goodmorning"], help="fun", description="says good morning"
    )
    ...
    async def howretarded(self, ctx, member: discord.Member = commands.Author):
        await ctx.reply(
            embed=discord.Embed(
                color=self.bot.color,
                title="how retarded",
                description=f"{member.mention} is {randrange(101)}% retarded",
            )
        )
    ...
    @commands.hybrid_command(
        description="cool rate an user", help="fun", usage="<member>"
    )
    ...
    @commands.hybrid_command(
        description="check an user's iq", help="fun", usage="<member>"
    )
    ...
    @commands.hybrid_command(
        description="hot rate an user", help="fun", usage="<member>"
    )
    ...
    @commands.hybrid_command(
        description="ship rate an user", help="fun", usage="[member]"
    )
    ...
    async def pack(self, ctx: commands.Context, *, member: discord.Member):
        if member == ctx.author:
            return await ctx.reply("Why do u wanna pack urself stupid boy?")
    ...
    async def bible(self, ctx: commands.Context):
        byte = await self.bot.session.read(
        ...
        embed = discord.Embed(
            color=self.bot.color, description=bible["text"]
        ).set_author(
            name="{} {}:{}".format(bible["bookname"], bible["chapter"], bible["verse"]),
