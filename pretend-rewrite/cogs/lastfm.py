...
import discord
import get.userhandler as userhandler
...
async def lastfm_message(ctx: commands.Context, content: str) -> discord.Message:
    return await ctx.reply(
        embed=discord.Embed(
            color=0xFF0000,
            description=f"> <:link:1243406673184424016> {ctx.author.mention}: {content}",
        )
    )
...
class Lastfm(commands.Cog):
    ...
    async def lf_set(self, ctx: commands.Context, *, ref: str):
        ...
        embed = discord.Embed(
            color=self.bot.color, description=f"> Connected to **{ref}** 🎸"
        )
        return await ctx.reply(embed=embed)

    @lastfm.command(
        name="remove", help="lastfm", description="unset your lastfm account"
    )
    ...
    @lastfm.command(
        name="variables",
        description="view lastfm custom embed variables",
        help="lastfm",
    )
    ...
    async def lf_embed_set(self, ctx: commands.Context, *, embed: str):
        ...
        if any(invite in ctx.message.content for invite in invites):
            await ctx.send_warning(
                "You cannot have discord invite links in your **last.fm** embed"
            )
    ...
    @lf_embed.command(
        name="view", description="check your lastfm custom embed", help="lastfm"
    )
    ...
    @lf_embed.command(
        name="none",
        description="clear your last.fm custom embed",
        help="lastfm",
        aliases=["delete"],
    )
    ...
    @lastfm.command(
        name="howto",
        help="lastfm",
        description="tutorial for using lastfm",
        aliases=["tutorial"],
    )
    ...
    @lastfm.command(
        name="crowns",
        help="lastfm",
        description="get the crowns of a member",
        usage="<user>",
    )
