...
from get.checks import Mod, Perms
from get.pretend import Pretend, PretendContext
from get.utils import EmbedBuilder, GoodRole, InvokeClass, NoStaff
...
class Moderation(commands.Cog):
    ...
    async def reactionmute(self, ctx, target: discord.Member, duration: int):
        ...
        await ctx.send(
            f"{target.mention} has been muted from reacting for {duration} minutes."
        )
        ...
        async def yes_callback(interaction: discord.Interaction):
            ...
            file = discord.File(
                fp=await self.bot.getbyte(
                    "https://media.tenor.com/oncTrVdxTDYAAAAC/nuke.gif"
                ),
                filename="nuke.gif",
            )
            await c.send("hello.")
    ...
    async def kick(
        ...
        *,
        ...
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot kick a bot owner.",
                color=0x2B2D31,
            )
    ...
    async def ban(
        ...
        *,
        ...
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot ban a bot owner.",
                color=0x2B2D31,
            )
            await ctx.send(embed=embed)
        else:
    ...
    async def mute(
        ...
        *,
        ...
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot mute a bot owner.",
                color=0x2B2D31,
            )
            await ctx.send(embed=embed)
        else:
    ...
    async def softban(
        ...
        *,
        ...
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot softban a bot owner.",
                color=0x2B2D31,
            )
            await ctx.send(embed=embed)
        else:
    ...
    async def voicemute(self, ctx: commands.Context, *, member: NoStaff):
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot voicemute a bot owner.",
                color=0x2B2D31,
            )
            await ctx.send(embed=embed)
        else:
            if not member.voice.channel:
                return await ctx.send_warning(
                    f"**{member}** is **not** in a voice channel"
                )
            if member.voice.self_mute:
                return await ctx.send_warning(
                    f"**{member}** is **already** voice muted"
                )
    ...
    async def jail(
        ...
        *,
        ...
        if member.id in self.bot.owner_ids:
            embed = discord.Embed(
                description=f"{ctx.author.mention}: You cannot jail a bot owner.",
                color=0x2B2D31,
            )
            await ctx.send(embed=embed)
        else:
            ...
            roles = [
                ...
                if r.name != "@everyone" and r.is_assignable()
            ]
