...
class EconomyCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.conn = sqlite3.connect('get/economy.db')
        ...
        # Load jobs from file
        with open('get/jobs.txt', 'r') as file:
    ...
    @commands.command(help="economy", description="check your balance account", aliases=['bal'])
    async def balance(self, ctx):
        ...
        if balances:
            ...
            await self._send_embed(ctx, f"You don't have an account yet. Use `{ctx.clean_prefix}register` to create one.")

    @commands.command(help="economy", description="make a account at pretend bank",)
    async def register(self, ctx):
        ...
        try:
            ...
            self.conn.commit()
            await self._send_embed(ctx, "You have been registered at the pretend bank successfully!")
        except sqlite3.IntegrityError:
            await self._send_embed(ctx, "You are already registered.")

    @commands.command(help="economy", description="rob a user",)
    ...
    @commands.command(help="economy", description="work and earn money",)
    ...
    @commands.command(help="economy", description="dumpster dive a bank", aliases=["dd"])
    async def dumpsterdive(self, ctx):
        ...
        self.cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (income, user_id))
        self.conn.commit()
        content = f"You decided to dumpster dive and found **{income}** :money_with_wings: u nasty perv.."
        await self._send_embed(ctx, content)

    @commands.command(help="economy", description="deposit money in your pretend bank account",)
    ...
    @commands.command(help="economy", description="withdraw money from your pretend bank account",)
    ...
    @commands.command(aliases=["richest"], help="economy", description="see the top reachest users in this guild",)
    ...
    @commands.command(help="economy", description="close your pretend bank account",)
    async def close(self, ctx):
        ...
        self.conn.commit()
        await self._send_embed(ctx, "Your pretend bank account has been closed.")

    @commands.command(help="economy", description="gamble your pretend money",)
    ...
    @commands.command(help="economy", description="give amount of money to a user",)
