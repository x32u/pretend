...
def get_progress(xp, level):
  corner_black_left = "<:white_left_rounded:1244747998358011955>"
  black = "<:white:1244748002396995705>"
  corner_black_right = "<:white_right_rounded:1244747993970769922>" 
  corner_white_left = "<:blue_left_rounded:1244747985951264884>"
  white = "<:blue:1244747989814087834>"
  corner_white_right = "<:blue_right_rounded:1244747981752897671>"
...
class Leveling(commands.Cog): 
  ...
  async def leaderboard(self, ctx: commands.Context):
    ...
    for table in results:
      ...
      level = table['level']
      num += 1
      auto += f"\n{'<:white_crown:1266123516416364705>' if num == 1 else f'`{num}`'} **{await self.bot.fetch_user(user) or user}** - **{exp}** xp (level {level})"
      k+=1
      l+=1
      if l == 10: break
    messages.append(auto)
