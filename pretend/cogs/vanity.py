...
class Vanity(commands.Cog):
    ...
    @commands.command(help="config", description="snipe vanity`s on a channel", brief="manage_channels", usage="[channel]")
    ...
    async def vanityset(self, ctx, channel: discord.TextChannel):
        ...
        if channel.id in vanity_channels:
            await ctx.send_warning("Vanity logging is already enabled for this channel.")
        else:
            self.save_vanity_channel(guild_id, channel.id)
            await ctx.send_success(f"Vanity logging enabled for #{channel.name}.")

    @commands.command(help="config", description="remove vanity snipe from a channel", brief="manage_channels", usage="[channel]")
    @utils.get_perms("manage_channels")
