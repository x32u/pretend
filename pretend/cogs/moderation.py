...
from get.checks import Mod
from get.utils import InvokeClass
from get.utils import EmbedBuilder, GoodRole, NoStaff
...
class ClearMod(discord.ui.View): 
  ...
  @discord.ui.button(emoji="<:check:1251929419795398708>")
  ...
  @discord.ui.button(emoji="<:deny:1251929424777969797>")
...
class Moderation(commands.Cog): 
  ...
  async def reactionmute(self, ctx, target: discord.Member, duration: int):
      muted_role = discord.utils.get(ctx.guild.roles, name="Muted")
      if not muted_role:
          muted_role = await ctx.guild.create_role(name="Pretend Reaction Mute")
        
      await target.add_roles(muted_role)
        
      await ctx.send(f"{target.mention} has been muted from reacting for {duration} minutes.")
        
      await asyncio.sleep(duration * 60)
      await target.remove_roles(muted_role)
        
      await ctx.send(f"{target.mention}'s mute has been lifted.")
    
  @commands.command()
  @Perms.get_perms("manage_roles")
  @Mod.is_mod_configured()
  async def reactionunmute(self, ctx, target: discord.Member):
      muted_role = discord.utils.get(ctx.guild.roles, name="Pretend Reaction Mute")
      ...
      if muted_role and muted_role in target.roles:
          ...
          await ctx.send(f"{target.mention} is not muted.")
  ...
  async def kick(self, ctx: commands.Context, member: NoStaff, *, reason: str="No reason provided"):
    if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot kick a bot owner.", color=0x2B2D31)
  ...
  async def ban(self, ctx: commands.Context, member: NoStaff, *, reason: str="No reason provided"):
   if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot ban a bot owner.", color=0x2B2D31)
      await ctx.send(embed=embed)
   else:
  ...
  async def mute(self, ctx: commands.Context, member: NoStaff, time: str="60s", *, reason="No reason provided"): 
    if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot mute a bot owner.", color=0x2B2D31)
      await ctx.send(embed=embed)
    else:
  ...
  async def softban(self, ctx: commands.Context, member: NoStaff, *, reason: str="No reason provided"):
    if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot softban a bot owner.", color=0x2B2D31)
      await ctx.send(embed=embed)
    else: 
  ...
  async def voicemute(self, ctx: commands.Context, *, member: NoStaff): 
    if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot voicemute a bot owner.", color=0x2B2D31)
      await ctx.send(embed=embed)
    else:
  ...
  async def jail(self, ctx: commands.Context, member: NoStaff, *, reason: str="No reason provided"):
    if member.id in self.bot.owner_ids:
      embed = discord.Embed(description=f"{ctx.author.mention}: You cannot jail a bot owner.", color=0x2B2D31)
      await ctx.send(embed=embed)
    else:
