...
class PretendHelp(HelpCommand):
  ...
  async def send_bot_help(self, mapping):
    ...
    embed.description = "> All commands are listed **[here](https://pretend.website/commands.html)**, join the discord server @ **[discord](https://discord.gg/pretendbot)**"
    options = []
    ...
    async def select_callback(interaction: Interaction):
      ...
      for c in [cmd for cmd in set(self.context.bot.walk_commands()) if cmd.help == select.values[0]]:
        if c.parent:
          ...
          cmds.append(c.name)
      e = Embed(color=self.context.bot.color, description=f"")
  ...
  async def send_command_help(self, command: Command):
    ...
    embed = Embed(color=self.bot.color, title=command.name, description=command.description)
    embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
    embed.add_field(name="module", value=command.help)
    embed.add_field(name="permissions", value=f"{command.brief if command.brief else 'any'}")
  ...
  async def send_group_help(self, group: Group):
    ...
    for command in group.commands:
      i+=1
      embed = Embed(color=self.bot.color, title=command.name, description=command.description)
      embed.add_field(name="usage", value=f"```{self.context.clean_prefix}{command.qualified_name} {command.usage if command.usage else ''}```")
      embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
      embed.set_footer(text=f"aliases: {', '.join(command.aliases)} pages: {i}/{len(group.commands)}")
      embeds.append(embed)
