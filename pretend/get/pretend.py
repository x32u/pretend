...
class Pretend(commands.AutoShardedBot):
    def __init__(self):
        super().__init__(
            ...
            owner_ids=[852784127447269396, 1188163689208090684, 1015714172195049502, 713128996287807600],
            activity=CustomActivity(name="🔗 pretend.website", state="🔗 pretend.website")
        )
        self.color = 0x729BB0
        self.warn = 0xefbc1b
        self.deny = 0xeb3434
        ...
        self.main_guilds = [1258712533284683798, 1246416801311625308, 1266088675880472669, 713128996287807600]
        self.proxy_url = "http://14a4a94eff770:c3ac0449fd@104.234.255.18:12323"
        self.no = "<:deny:1280595851370037361>"
        self.yes = "<:approve:1280595763537117317>"
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
      if isinstance(error, commands.CommandNotFound): return 
        ...
        if isinstance(error, commands.MissingPermissions): return await ctx.send_warning(f"You are **missing** the following permission: `{error.missing_permissions[0]}`")
      elif isinstance(error, commands.CommandOnCooldown):
        if ctx.command.name != "hit": return await ctx.reply(embed=discord.Embed(color=0xefbc1b, description=f"{ctx.author.mention}: Wait **{format_timespan(error.retry_after)}** before using **{ctx.command.qualified_name}** again"), mention_author=False)    
       ...
       await self.ext.send_warning(ctx, f"You've encountered an error, please join our [**support server**](https://discord.gg/V6w2X8wuwx) and provide us with the code: `{key}`")   
...
def command_with_category(*args, **kwargs):
    category = kwargs.pop('category', 'general')
    cmd = commands.command(*args, **kwargs)
    def decorator(func):
        result = cmd(func)
...
def get_command_details():
    ...
    for command in bot.commands:
        ...
        commands_list.append(cmd_details)
    return commands_list

@bot.event
async def on_ready():
    commands_details = get_command_details()
    with open('cmds.json', 'w') as f:
        json.dump(commands_details, f, indent=4)
    print('done')
